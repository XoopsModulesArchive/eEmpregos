...
function index()
{
    ...
    if ('1' == $moderated) {
        $result = $xoopsDB->query('select  COUNT(*)  FROM ' . $xoopsDB->prefix('eEmpregos_annonces') . " WHERE valid='No'");
    ...
    while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
        ...
        $a_cat['image'] = "<img src='" . XOOPS_URL . '/modules/eEmpregos/images/cat/' . $myrow['img'] . "' align='absmiddle'>";

        $a_cat['link'] = '<a href="index.php?pa=view&cid=' . $myrow['cid'] . "\"><b>$image</b></a>";
    ...
    [$ann] = $xoopsDB->fetchRow($xoopsDB->query('select  COUNT(*)  FROM ' . $xoopsDB->prefix('eEmpregos_annonces') . " WHERE valid='Yes'"));
...
function view($cid, $debut)
{
    ...
    [$nbe] = $xoopsDB->fetchRow($xoopsDB->query('select COUNT(*) FROM ' . $xoopsDB->prefix('eEmpregos_annonces') . " where valid='Yes' AND cid='$cid'"));
    ...
    if (0 != $numrows) {
        ...
        while (list($ccid, $title, $img) = $xoopsDB->fetchRow($subresult)) {
            ...
            $a_cat['image'] = "<img src='" . XOOPS_URL . "/modules/eEmpregos/images/cat/$img' align='absmiddle'>";
...
function viewannonces($lid)
{
    ...
    $result = $xoopsDB->query('select lid, cid, title, type, company, description, tel, price, typeprix, date, email, submitter, usid, town, country, valid, photo, view FROM ' . $xoopsDB->prefix('eEmpregos_annonces') . " WHERE lid = '$lid'");
    ...
    $requete2 = $xoopsDB->query('select cid from ' . $xoopsDB->prefix('eEmpregos_annonces') . ' where  lid=' . $lid . '');
    ...
    if ($recordexist) {
        [$lid, $cid, $title, $type, $company, $description, $tel, $price, $typeprix, $date, $email, $submitter, $usid, $town, $country, $valid, $photo, $view] = $xoopsDB->fetchRow($result);

        //	Specification for Japan: move after for view count up judge

        //		$xoopsDB->queryf("UPDATE ".$xoopsDB->prefix("eEmpregos_annonces")." SET view=view+1 WHERE lid = '$lid'");
        ...
        $xoopsTpl->assign('company_head', _CLA_COMPANY2);

        $xoopsTpl->assign('description_head', _CLA_DESC2);
    ...
    $xoopsTpl->assign('link_main', '<a href="../eEmpregos/">' . _CLA_MAIN . '</a>');
...
function categorynewgraphic($cat)
{
    ...
    $newresult = $xoopsDB->query('select date from ' . $xoopsDB->prefix('eEmpregos_annonces') . " where cid=$cat and valid = 'Yes' order by date desc limit 1");
