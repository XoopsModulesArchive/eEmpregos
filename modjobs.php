...
function AnnoncesDel($lid, $ok)
{
    ...
    $result = $xoopsDB->query('select usid, photo FROM ' . $xoopsDB->prefix('eEmpregos_listing') . " where lid=$lid");
    ...
    if ($xoopsUser) {
        ...
        if ($usid == $calusern) {
            if (1 == $ok) {
                $xoopsDB->queryF('delete from ' . $xoopsDB->prefix('eEmpregos_listing') . " where lid=$lid");
            ...
            echo "[ <a href=\"modeEmpregos.php?op=AnnoncesDel&amp;lid=$lid&amp;ok=1\">" . _JOBS_OUI . '</a> | <a href="index.php">' . _JOBS_NON . '</a> ]<br><br>';
...
function ModAnnonce($lid)
{
    ...
    $result = $xoopsDB->query('select lid, cid, title, type, company, description, requirements, tel, price, typeprice, contactinfo, date, email, submitter, usid, town, valid, photo from ' . $xoopsDB->prefix('eEmpregos_listing') . " where lid=$lid");
    ...
    if ($xoopsUser) {
        ...
        if ($usid == $calusern) {
            ...
            echo '<form action="modeEmpregos.php" method=post ENCTYPE="multipart/form-data">
...
function ModAnnonceS($lid, $cat, $title, $type, $company, $description, $requirements, $tel, $price, $typeprice, $contactinfo, $date, $email, $submitter, $town, $valid, $photo, $photo_old, $photoS_size, $photoS_name, $HTTP_POST_FILES, $supprim)
{
    ...
    if (!empty($HTTP_POST_FILES['photo']['name'])) {
        ...
        if (!$upload->upload()) {
            redirect_header("modeEmpregos.php?op=ModAnnonce&amp;lid=$lid", 3, $upload->getErrors());
